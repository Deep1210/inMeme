...
export default class HomeScreen extends Component {
    ...
    getMemes(id) {
        ...
        if (id) {
            fetch(`http://207.246.125.54/api/meme?device=${uniqueId}&category=${id}`, {
                method: 'GET'
            }).then(response => {
                ...
                if (responseJson.results.length > 0) {
                    ...
                    this.setState({ memeData: responseJson.results, loading: false, noMoreContent: true })
                }
            })
        } else {
    ...
    clickLike(){
        ...
        fetch(`http://207.246.125.54/api/meme/${this.state.memeData[this.state.currentIndex].id}/like-unlike/`, {
            method: 'post',
            headers: {
    ...
    render() {
        ...
        return (
            <View style={{ flex: 1 }}>
                ...
                {this.state.show || this.state.noMoreContent ?
                    <View style={{ backgroundColor: 'white', flex: 1, alignItems: 'center', flexDirection: 'row' }}>
                        ...
                        <TouchableOpacity onPress={() => this.openDrawer()} style={{ marginLeft: '84%' }}>
                            ...
                            <ModalDropdown dropdownStyle={{ width: 100, height: 110 }}
                ...
                {(this.state.show && !this.state.noMoreContent) ? <View style={{ backgroundColor: 'white', flex: 1, flexDirection: 'row', justifyContent: 'space-evenly', alignItems: 'center' }}>
